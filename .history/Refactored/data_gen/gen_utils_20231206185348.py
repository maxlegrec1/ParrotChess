...
def load_games(L_pgn):
    L = [load_game(pgn) for pgn in L_pgn]
...
def gen_dataset(nb_data, batch_size, nb_process):
    ...
    with open(in_pgn) as f:
        for iteration in range(nb_iterations):
            ...
            res = [ ray.remote(load_games)(batch) for batch in batches]
            data = ray.get(res)
            x = np.array([data[i][0] for i in range(len(data))])
            y = np.array([data[i][1] for i in range(len(data))])
            x_data.append(x)
            y_data.append(y)
    x_fulldata = np.concatenate(x_data, axis=0)
    y_fulldata = np.concatenate(y_data, axis=0)
    np.save(f"Refactored/RNN/x{len(x_fulldata)}.npy")
    np.save(f"Refactored/RNN/y{len(y_fulldata)}.npy")
if __name__ == "__main__":
    gen_dataset(1000,10,10)
