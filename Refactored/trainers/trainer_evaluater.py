...
@tf.function
def train_step(batch,model,evaluater):
    with tf.GradientTape() as tape:
        ...
        for i in range(5):
            played_moves = tf.argmax(y_true,axis=-1)
            resulting_positions = get_resulting_positions(fens,played_moves)
            resulting_values = evaluater(resulting_positions)
            x[3][i] = played_moves
            x[4][i] = resulting_values
            y_pred = model(x)
            loss =tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(y_true),logits=y_pred)
            ...
            gradients = tape.gradient(loss, evaluater.trainable_variables)
            #clip gradients
            gradients, _ = tf.clip_by_global_norm(gradients, 10000)
...
def train(gen, model,evaluater, num_step, lr_start):
    ...
    for epoch in range(10000):
        ...
        for step in range(num_step):
            ...
            loss,lm,acc = train_step(batch, model,evaluater)
