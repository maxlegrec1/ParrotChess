...
def create_body(inputs,num_residuals=16, num_filters = 64,mini_res_channels = 64):
    flow_input = tf.keras.layers.concatenate([inputs[0],inputs[2],inputs[3]],axis=-1)
    flow = conv_block(flow_input,
                        filter_size=3,
                        output_channels=num_filters,
                        name='input',
                        bn_scale=True)
    #mini_residual = residual_model(mini_res_channels,name=f'mini_residual_{i}')
...
def create_model(params):
    ...
    input3 = tf.keras.layers.Input(shape=(8,8,(12*num_iters)))
    input4 = tf.keras.layers.Input(shape=(8,8,num_iters))
    x = create_body([input1,input2,input3,input4],num_residuals=nb_residuals, num_filters = num_filters, mini_res_channels = mini_res_channels )
    ...
    model = tf.keras.Model(inputs=[input1,input2,input3,input4], outputs=h_fc1)
