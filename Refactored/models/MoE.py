...
class TFProcess:
    ...
    def construct_net(self, inputs, name: str = ""):
        ...
        top10,_ = tf.math.top_k(policy,k=10)
        last = top10[:,-1]
        mask = policy>=last
        def expert(name,flow):
            
            for i in range(self.encoder_layers):
                self.encoder_layer(flow, self.embedding_size, self.encoder_d_model,
                                                    self.encoder_heads, self.encoder_dff,
                                                    name=name+"encoder_{}".format(i + 1), training=True)
            ...
            logit = tf.keras.layers.Dense(1,
                                          kernel_initializer="glorot_normal",
                                          kernel_regularizer=self.l2reg,
                                          activation="sigmoid",
                                          name=name+"/dense1")(h_fc2)
            
            return logit
        logits = [expert(f"expert_{i}",flow_before) for i in range(policy.shape[-1])]

        #logits = [experts[i](flow_before) for i in range(policy.shape[-1])]

        logits = tf.concat(logits,axis=-1)*mask
