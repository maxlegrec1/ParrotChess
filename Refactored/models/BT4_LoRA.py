...
def ma_gating(inputs, name):
    out = Gating(name=name + '/mult_gate', additive=False,trainable = False)(inputs)
    out = Gating(name=name + '/add_gate', additive=True,trainable = False)(out)
    return out
...
class RMSNorm(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        self.gamma = self.add_weight(name="gamma",
                                     shape=[input_shape[-1]],
                                     initializer="ones",
                                     trainable=False) if self.scale else 1
...
if __name__=='__main__':
    model = create_model()
    model.summary()
    from BT4 import create_model as BT4_create_model
    BT4 = BT4_create_model()
    BT4.load_weights("BT4_blank_weights.h5")
