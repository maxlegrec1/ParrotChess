            ...
            if pgn.next()!=None:
                ...
                if len(moves)>=11 and random.random() < uniform_density()/(4*weibull_min.pdf(elo,3.781083215802374, 355.0827163803461, 1421.9764397854142)):
                    ...
                    for i,move in enumerate(moves[:start_index]):
                        del xs,ys
                        xs,ys, vs = get_board_data(pgn,board,move,i)
                    ...
                    for i,move in enumerate(moves[start_index:]):
                        ...
                        del xs,ys,vs
                        xs,ys,vs = get_board_data(pgn,board,move,start_index+i)
                        if i == 0 :
...
def get_board(elo,board,real_move,TC,move_number,value):
    if board.turn == chess.WHITE:
        color = 1
    ...
    del mirrored_board
    return np.concatenate((before,castling_rights,en_passant_right,color,TC,elo),axis=2),one_hot_move,value
