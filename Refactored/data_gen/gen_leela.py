...
def board_to_input_data(board: chess.Board) -> List[np.ndarray]:
    ...
    input_data.append(np.zeros((8, 8),dtype=np.float32))
        ...
        while True:
            ...
            if pgn.next()!=None:
                ...
                try: 
                    ...
                    elo = 1500
                if len(moves)>=11 and elo>2400 and random.random() < uniform_density()/(4*weibull_min.pdf(elo,3.781083215802374, 355.0827163803461, 1421.9764397854142)):
                    ...
                    for i,move in enumerate(moves):
                        ...
                        if i%2 == 0:
                            ...
                            y_true.append(one_hot_move)

                        else:
                            mirror_uci = mirror_uci_string(move.uci())
                            mmove = chess.Move.from_uci(mirror_uci)
                            if mmove.uci()[-1] != 'n':
                                move_id = policy_index.index(mmove.uci())
                            else:
                                move_id = policy_index.index(mmove.uci()[:-1])

                            mirrored_board = chess_board.mirror()
                            lm = - np.ones(1858, dtype=np.float32)
                            for possible in mirrored_board.legal_moves:
                                possible_str = possible.uci()
                                if possible_str[-1] != 'n':
...
def generator_uniform(generator,batch_size):
    while True:
        ...
        for i in range(batch_size):
            ...
            Xs = np.concatenate([Xs_8,Xs_7,Xs_6,Xs_5,Xs_4,Xs_3,Xs_2,Xs_1,rest],axis = -1)
            Xs = tf.transpose(Xs,perm=[0,3,1,2])
...
def generator_uniform2(generator, batch_size):
    while True:
        x,y = next(generator)
        yield x,y
...
class data_gen():
    def __init__(self, params):
        ...
        self.gen = generator_uniform2(generate_batch(batch_size,pgn),batch_size)
